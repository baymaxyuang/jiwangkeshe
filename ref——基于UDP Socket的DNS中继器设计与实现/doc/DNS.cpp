...
int main()
{
	...
	std::thread dns_consulting_threads2(DNSHandleThread, "10.3.9.4", listen_socket, 2);
	std::thread dns_consulting_threads3(DNSHandleThread, "10.3.9.4", listen_socket, 3);
	std::thread dns_consulting_threads4(DNSHandleThread, "10.3.9.4", listen_socket, 4);
	...
	do
	{
		...
		if (iResult == SOCKET_ERROR) {
			...
			//std::thread dns_consulting_threads3(DNSHandleThread, "10.3.9.4", listen_socket, 1);
...
void DNSReturnThread(SOCKET upper_dns_socket, SOCKET listen_socket, int t_id)
{
	...
	while(1)
	{
		...
		{
			THREADDEBUG << "Bytes received from ***: " << sendbuflen << std::endl;		//We received something.
